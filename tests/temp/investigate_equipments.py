    ...
    try:
        ...
        if isinstance(data1, dict):
            ...
            if "results" in data1 and data1["results"]:
                ...
                print(
                    f"  - Chaves: {list(first_result.keys()) if isinstance(first_result, dict) else 'não é dict'}"
                )
                ...
                if "equipamentos" in first_result and first_result["equipamentos"]:
                    ...
                    print(
                        f"  - Chaves: {list(primeiro_equip.keys()) if isinstance(primeiro_equip, dict) else 'não é dict'}"
                    )

                    # Pegar ID para testar a segunda API
                    equip_id = (
                        primeiro_equip.get("id") if isinstance(primeiro_equip, dict) else None
                    )
                    ...
                    if equip_id:
                        ...
                        if "results" in data1:
                            for company in data1["results"][
                                :3
                            ]:  # Mostrar apenas 3 primeiras empresas
                                if "equipamentos" in company:
                                    print(
                                        f"  - Empresa '{company.get('nome', 'sem nome')}': {len(company['equipamentos'])} equipamentos"
                                    )
