...
from app.services.sync.delta import IncrementalSync, run_incremental_sync, should_run_incremental_sync
...
class TestBackfillSync:
    ...
    async def test_sync_orders_basic(self):
        ...
        # Mock client
        mock_client = Mock()
        mock_client.list_chamados = AsyncMock(return_value=[
...
class TestIncrementalSync:
    ...
    def test_should_run_incremental_sync(self):
        ...
        conn.execute(
            "UPDATE sync_state SET synced_at = ? WHERE resource = ?",
            (old_time, 'fresh_resource')
        )
        conn.commit()
